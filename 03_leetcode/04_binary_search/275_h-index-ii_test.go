...
func TestHIndexII(t *testing.T) {
	convey.Convey("H 指数 II:citations 已经按照 非降序排列 , "+
		"h 代表“高引用次数”（high citations），一名科研人员的 h 指数是指他（她）的 （n 篇论文中）至少 有 h 篇论文分别被引用了至少 h 次", t, func() {
		testCase := []struct {
			input    []int
			expected int
		}{
...
func hIndex(citations []int) int {
	/*
		如果至少有 2 篇论文引用次数 >=2, 那么 至少 1 篇 引用次数 >=1;

		如果没有 4 篇论文引用次数 >=4, 那么 没有 5 篇 引用次数 >=5;
	*/

	// 引用的次数 在区间 [left, right] 内询问
